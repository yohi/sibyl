...
- `LayoutManager`: 再帰的な Flexbox ペインレイアウト（Solid.js）。
- `Pane`: 1 つのペインを表す Solid コンポーネント。
- `PaneBackend`: OpenTUI 実装と外部アダプターのための抽象。
- `OpenTuiPaneBackend`: OpenTUI + PTY 版の実装。
...
- **ペイン単位 PTY サイズ**: OpenTUI Solid は現時点でペイン単位のサイズ API を提供していないため、各 `Pane` は端末全体の `cols`/`rows` を PTY リサイズに使用しています。分割ペインも同一サイズを共有する状態になります。API 提供時にペイン単位サイズへ移行する予定です。
